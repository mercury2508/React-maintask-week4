...
function ProductModal({isProductModalOpen, setIsProductModalOpen, modalState, tempProduct, getProducts}){
    ...
    const adjustProduct = async () => {
        ...
        await axios.put(`${baseUrl}/api/${apiPath}/admin/product/${modalData.id}`, productData)
        alert("已編輯產品");
        closeModal();
        } catch (error) {
        alert(`產品編輯失敗:${error}`);
                            ...
                            <div className="col-md-4">
                                ...
                                <div className="border border-2 border-dashed rounded-3 p-3">
                                    ...
                                    <div className="btn-group w-100">
                                        {modalData.imagesUrl.length < 5 && modalData.imagesUrl[modalData.imagesUrl.length - 1] !== "" && (<button className="btn btn-outline-primary btn-sm w-100" onClick={handleAddImages}>新增圖片</button>)}
                                        {modalData.imagesUrl.length > 1 && (<button className="btn btn-outline-danger btn-sm w-100" onClick={handleRemoveImages} >取消圖片</button>)}
                                    </div>
                                </div>
                            </div>
